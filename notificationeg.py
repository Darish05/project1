...
def fetch_due_tasks():
    ...
    query = "SELECT email, title, due_date, due_time FROM tasks INNER JOIN users ON tasks.user_id = users.id WHERE due_date = %s AND due_time BETWEEN %s AND %s"
    cursor.execute(query, (current_time.date(), current_time.time(), one_hour_later.time()))
...
def send_email_notification(email, task):
    sender_email = "darishjerlin2005@gmail.com"
    sender_password = "gkwf waob nyzj vsek"
    receiver_email = email

    message = MIMEMultipart()
    message["From"] = sender_email
    message["To"] = receiver_email
    message["Subject"] = "Task Reminder: Your task is due soon!"

    body = f"Hello,\n\nYour task '{task[1]}' is due on {task[2]} at {task[3]}.\n\nRegards,\nTask Manager App"
...
# Function to send due task notifications
def send_due_task_notifications():
    tasks = fetch_due_tasks()
    for task in tasks:
        send_email_notification(task[0], task)

# Schedule the job to run periodically
def schedule_email_notifications():
    schedule.every().hour.do(send_due_task_notifications)
